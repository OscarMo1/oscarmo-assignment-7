...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    error = np.random.normal(0, np.sqrt(sigma2), N)  # Error term with mean 0 and variance sigma2
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color="red", label="Fit line")
    plt.legend()
...
def confidence_interval():
    ...
    t_value = t.ppf(1 - (1 - confidence_level) / 2, df=S - 1)
